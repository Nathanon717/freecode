...
import { mkdtempSync, writeFileSync, existsSync, mkdirSync, unlinkSync } from 'fs';
...
const sockPath = (id: string) => join(SESSION_DIR, `${id}.sock`);
...
async function runServer(id: string, cols: number, rows: number): Promise<void> {
  ...
  process.on('exit', () => {
    try { unlinkSync(sp); } catch { /* already gone */ }
    try { unlinkSync(fp); } catch { /* already gone */ }
  });

  server.listen(sp, () => writeFileSync(fp, ''));
...
function rpc(id: string, msg: object): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
    const socket = net.createConnection(sockPath(id));
    let buf = '';
...
async function cmdStart(cols: number, rows: number): Promise<void> {
  ...
  spawn(tsx, [self, '--server', id, '--cols', String(cols), '--rows', String(rows)], {
...
async function cmdStop(id: string): Promise<void> {
  await rpc(id, { type: 'stop' }).catch(() => { /* session may already be dead */ });
  console.log('stopped');
}
