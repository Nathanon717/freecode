...
### Checkpoint snapshots in e2e tests

Every write tool call takes a checkpoint snapshot of the workspace before it runs (see
[commands.md](commands.md#checkpoint-freecode-checkpoint)). The harness points every child at **one**
...
only `checkpoint-snapshot-on-first-write` does — sets `"env": { "FREECODE_SNAPSHOT_DIR": "" }` so its
