...
1. Add provider config to `src/providers/provider-catalog.ts` with:
...
2. Every provider goes through `createOpenAICompatProvider()`. There is no second
   ...
   compatibility endpoint (as Anthropic has) rather than a bespoke adapter.
   Per-provider deviations belong in `src/providers/adapters/openai-compat-quirks.ts`.
...
Anthropic is reached through its official OpenAI-compatibility endpoint
(`https://api.anthropic.com/v1`), so it is an ordinary entry in the catalog with
no adapter of its own. The compatibility layer covers chat completions, tools,
streaming, vision, and `usage` token counts; it does **not** support prompt
caching, so no cache-tier accounting exists anywhere in the codebase. There is no
per-turn cost display for any provider — `/model` shows verified per-million
rates instead (see `src/providers/pricing-verifier.ts`).
