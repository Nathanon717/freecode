...
- `cli/menus/raw-picker.ts` — provides `runRawKeySession` for the stdin lifecycle.
- `cli/chrome/bottom-ui.ts` — footer/bottom-UI state queried for absolute positioning and for the preview row budget.
