...
- `VIEWPORT_SIZE` / `clampViewport` — shared by tab bodies that scroll a long item list (`custom-eval-menu.ts` Custom tab, `cli/humaneval-menu.ts` HumanEval tab).
...
- `cli/custom-eval-menu.ts`, `cli/humaneval-menu.ts`, `commands/config.ts`, `commands/model.ts` — adopters.
