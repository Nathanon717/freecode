...
- `cli/eval/custom-eval-menu.ts` — `/eval` picker
- `cli/chrome/bottom-ui.ts` — `drawFooter` called after each redraw
- `cli/tools/tool-approval.ts` — uses `runRawKeySession` for the Approve/Deny menu and line-editor (Phase 2 done)
