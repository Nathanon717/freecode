...
- Printable input goes through `handlePrintable`: typing `(` right after a valid tool name (see `cli/tool-invocation.ts`) autofills the full argument skeleton (`read(path="", offset=, limit=)`) with the caret in the first value slot; typing `)` on top of a `)` types over it. Tab cycles forward through the value slots and Backspace at an emptied slot steps back (via `nextToolFieldCaret` / `toolFieldBackspace`); on submit `stripEmptyToolArgs` drops untouched slots.
