...
**Key neighbors:** `src/config/index.ts`, `src/eval/custom.ts`, `src/eval/humaneval-data.ts`, `src/agent/system-prompt.ts`, `src/tokenizers/backends/bpe-json.ts`, `src/cli/custom-eval-menu.ts`, `scripts/generate-docs.ts`, `tests/harness/run-scenarios.ts`, `tests/repo-encoding.test.ts` (the repo-wide guard that uses `hasBom`).
