 ...
 *
 * Only the two id blocklists are considered. `modelTierBlocklist` matches on a tier
 * field that is never stored on the row, so it cannot be re-derived from the DB.
 *
...
export function findBlocklistedStoredModels(): BlocklistedStoredModel[] {
  ...
  for (const provider of PROVIDER_REGISTRY) {
    ...
    if (substrings.length === 0 && exact.size === 0) continue;
    for (const [key, entry] of Object.entries(store)) {
      if (entry.provider !== provider.id) continue;
      if (!exact.has(entry.modelId) && !substrings.some((b) => entry.modelId.includes(b))) continue;
      found.push({
        key,
